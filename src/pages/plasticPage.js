...
function Plastic() {
    ...
    useEffect(() => {
        ...
        const initialMenuList = getCategoryMenuList('플라스틱'); 
        setMenuList(initialMenuList);
    
        // 로컬 스토리지에서 추가된 메뉴 불러오기
        const storedAddedMenus = localStorage.getItem('addedMenus');
        if (storedAddedMenus) {
            const filteredMenus = JSON.parse(storedAddedMenus).filter(menu => menu.categoryName === '플라스틱'); 
    ...
    return (
        <div>
            ...
            <div className={boxStyle.pd}>
                <h1 className={boxStyle.center}>플라스틱 쓰레기</h1> 
            ...
            {showWriteForm && <RecycleWriteForm categoryName="플라스틱" onPostSubmit={addNewMenu} />}
        </div>
    );
}
